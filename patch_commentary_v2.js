...
// 1. Check if we already injected this (in case of re-runs)
if (!js.includes('let specialCommentaryHtml')) {
    const targetRegex1 = /const singleAwardHtml = `/;
    if (targetRegex1.test(js)) {
        // Replace the LAST singleAwardHtml declaration since it's the one in the loop
        const lastIndex = js.lastIndexOf('const singleAwardHtml = `');
        if (lastIndex !== -1) {
            js = js.slice(0, lastIndex) + codeToInject + '\n                    ' + js.slice(lastIndex);
    ...
    const targetRegex2 = /<\/div>\s*<\/div>\s*<div class="overflow-x-auto">/g;
    // We want to match the last occurrence in the template loop
    const matchIter = js.matchAll(targetRegex2);
    let lastMatch = null;
    ...
    if (lastMatch) {
        js = js.slice(0, lastMatch.index) + 
             '</div>\n                            </div>\n                            \\${specialCommentaryHtml}\n                            <div class="overflow-x-auto">' + 
             js.slice(lastMatch.index + lastMatch[0].length);
    ...
    console.log('Successfully patched generate_html.js with commentary.');
} else {
    console.log('specialCommentaryHtml is already in the file. No changes made.');
}
