...
// Read the original fetched CSV
const originalCsvPath = path.join(__dirname, '..', '..', 'brain', '991a5cf6-de4b-4f16-a27c-3b4b3f0b2984', '.system_generated', 'steps', '64', 'content.md');

// We parse the markdown/CSV content. The original file has header metadata, then a separator "---", then the CSV text.
let rawContent = fs.readFileSync(originalCsvPath, 'utf-8');
const separatorIndex = rawContent.indexOf('---');
if (separatorIndex !== -1) {
    rawContent = rawContent.substring(separatorIndex + 3).trim();
}

// Map partners to their found podcast names, Apple URLs, and RSS feeds
const mapping = {
    "美股夢想家": {
        podcast: "夢想家說股事",
...
for (let i = 0; i < lines.length; i++) {
    ...
    // We split by comma, but need to handle potential quoted fields containing commas
    const cells = [];
    let currentCell = '';
    let inQuotes = false;
    ...
    // If it's a data row (usually has number or partner name)
    // The partner name is in column index 2 (0-based)
    if (i >= 2) { // Skip header rows
        const partnerNameRaw = cells[2] || '';
        const partnerName = partnerNameRaw.replace(/"/g, '').trim();
        
        if (mapping[partnerName]) {
            const data = mapping[partnerName];
            // Cell index 9: Podcast節目名稱, index 10: Apple Podcast連結, index 11: Firstory RSS連結
            cells[9] = `"${data.podcast}"`;
            cells[10] = `"${data.apple}"`;
            cells[11] = `"${data.rss}"`;
        }
    }
...
const outputPath = path.join(__dirname, 'updated_sheet.csv');
fs.writeFileSync(outputPath, updatedLines.join('\n'), 'utf-8');
console.log(`已成功將更新後的 CSV 寫入到 ${outputPath}`);
