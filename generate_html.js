...
function generateSelfContainedHtml() {
    ...
    let trackCResults = null;
    const trackCResultsPath = path.join(__dirname, 'track_c_results.json');
    if (fs.existsSync(trackCResultsPath)) {
        try {
            trackCResults = JSON.parse(fs.readFileSync(trackCResultsPath, 'utf-8'));
            console.log("成功加載 Track C 評分與留言數據。");
        } catch (e) {
            console.error("讀取 track_c_results.json 失敗：", e.message);
        }
    }
...
<div id="content-timeline" class="prose max-w-none hidden">
        ...
        <div id="content-track-c" class="hidden w-full space-y-6">
            ...
            <div class="glass-card rounded-2xl p-6 sm:p-10">
                ...
                <div id="reviews-container" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="col-span-1 md:col-span-2 text-center py-8 text-slate-400 text-xs">
                        載入留言數據中...
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        window.trackCResults = ${JSON.stringify(trackCResults)};
    </script>
    ...
    <script>
        ...
        // Render Apple Podcast Customer Reviews
        function renderReviews() {
            const container = document.getElementById('reviews-container');
            if (!container) return;
            
            const trackCData = window.trackCResults || [];
            if (trackCData.length === 0) {
                container.innerHTML = '<div class="col-span-2 text-center py-6 text-slate-400 text-xs">無評分與留言數據</div>';
                return;
            }

            // Sort by reviewsCount descending, then averageRating descending
            const sortedData = [...trackCData].sort((a, b) => {
            ...
            sortedData.forEach((item, idx) => {
                ...
                if (item.reviews && item.reviews.length > 0) {
                    ...
                    const sortedReviews = [...item.reviews].sort((a, b) => new Date(b.date) - new Date(a.date));
                    sortedReviews.forEach((rev, rIdx) => {
                        const isHidden = rIdx >= 2 ? 'hidden' : '';
                        let revStars = '';
                        for (let s = 1; s <= 5; s++) {
                            if (s <= rev.rating) revStars += '★';
                            else revStars += '☆';
                        }
                        reviewsListHtml += \`
                            <div class="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-1 text-[11px] review-item-\${idx} \${isHidden}">
                                <div class="flex justify-between items-center text-[10px] text-slate-400 font-semibold">
                                    <span class="text-slate-600 font-bold">\${rev.author}</span>
                                    <span>\${rev.date || 'Unknown'}</span>
                                </div>
                                <div class="flex items-center space-x-1.5">
                                    <span class="text-amber-500 font-extrabold text-[10px] bg-amber-50 px-1 rounded">\${revStars}</span>
                                    <span class="font-bold text-slate-800">\${rev.title || '無標題'}</span>
                                </div>
                                <p class="text-slate-600 leading-relaxed font-sans-tc m-0 whitespace-pre-wrap">\${rev.content || '無內容'}</p>
                            </div>
                        \`;
                    });
                } else {
                    reviewsListHtml = '<div class="text-center py-4 text-slate-400 text-xs">過去 6 個月無公開留言</div>';
                }

                let toggleBtnHtml = '';
                if (item.reviews && item.reviews.length > 2) {
                    toggleBtnHtml = \`
                        <button onclick="toggleMoreReviews(\${idx}, this)" class="w-full py-1.5 text-center text-xs font-bold text-indigo-600 hover:text-indigo-800 bg-indigo-50/50 hover:bg-indigo-50 rounded-lg transition-all mt-2.5">
                            展開更多留言 (+\${item.reviews.length - 2} 則)
                        </button>
                    \`;
                }

                html += \`
                    <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between hover:border-slate-300 hover:shadow-md transition-all duration-200">
                        <div class="space-y-4">
                            <!-- Header -->
                            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start border-b pb-3 space-y-2 sm:space-y-0">
                                <div>
                                    <div class="flex items-center space-x-1.5">
                                ...
                                <div class="text-left sm:text-right shrink-0">
                                    <div class="flex items-center space-x-1 text-xs justify-start sm:justify-end">
                                        <span class="font-bold text-slate-800">\${item.averageRating || 'N/A'}</span>
                                        <span class="text-amber-400 text-xs font-black select-none tracking-tight">\${starsHtml}</span>
                                    </div>
                                    <span class="inline-block mt-1 text-[10px] font-bold bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full border border-emerald-100">
                                        💬 \${item.reviewsCount} 則留言
                                    </span>
                                </div>
                            </div>
        ...
        window.toggleMoreReviews = function(idx, btn) {
            const items = document.querySelectorAll('.review-item-' + idx);
            ...
            if (isExpanding) {
                btn.textContent = '收起留言';
            } else {
                btn.textContent = '展開更多留言 (+' + (items.length - 2) + ' 則)';
            }
        }
