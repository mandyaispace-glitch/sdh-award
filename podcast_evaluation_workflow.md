...
*   **核心執行腳本**：`poc_run.js`, `build_episode_pool.js`, `track_b_run.js`, `track_c_run.js`, `daily_ranking_logger.js`, `generate_html.js`, `update_csv.js`
...
### 3. 環境變數設定
在專案根目錄下建立 `.env` 檔案，寫入客戶專屬的 Gemini API Key：
```env
...
*   **執行決審橫向相對 PK**：`node poc_run.js`
*   **產出網頁規劃書報告**：`node generate_html.js`
