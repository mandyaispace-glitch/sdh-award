...
// Insert declaration before const singleAwardHtml
const insertIdx1 = js.indexOf('const singleAwardHtml =');
if (insertIdx1 !== -1) {
    js = js.slice(0, insertIdx1) + codeToInject + '\n                    ' + js.slice(insertIdx1);
} else {
    console.error("Failed to find 'const singleAwardHtml ='");
}

// Insert rendering before overflow-x-auto
const insertIdx2 = js.indexOf('<div class="overflow-x-auto">');
if (insertIdx2 !== -1) {
    js = js.slice(0, insertIdx2) + '${specialCommentaryHtml}\n                            ' + js.slice(insertIdx2);
} else {
    console.error("Failed to find '<div class=\"overflow-x-auto\">'");
}

fs.writeFileSync('generate_html.js', js);
console.log('Successfully patched generate_html.js with indexOf');
