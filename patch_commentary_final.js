...
if (insertIdx1 !== -1 && !js.includes('let specialCommentaryHtml =')) {
    js = js.slice(0, insertIdx1) + codeToInject + '\n                    ' + js.slice(insertIdx1);
    console.log("Injected specialCommentaryHtml declaration");
} else {
    console.error("Failed to find or already injected declaration.");
}

// Check if rendering was injected with escaped $ symbol
// We need to fix it from \${specialCommentaryHtml} to ${specialCommentaryHtml}
const escapedTarget = '\\${specialCommentaryHtml}';
if (js.includes(escapedTarget)) {
    // This removes the backslash so Node evaluates it and injects it into the HTML
    js = js.replace(escapedTarget, '${specialCommentaryHtml}');
    console.log("Fixed specialCommentaryHtml rendering to correctly interpolate");
}

fs.writeFileSync('generate_html.js', js);
console.log('Successfully patched generate_html.js with final commentary logic.');
