                        ...
                        <div class="bg-indigo-50/70 p-5 rounded-xl border border-indigo-100 mb-2 mt-4">
                            ...
                            <div class="text-indigo-800 space-y-3 text-xs leading-relaxed">
                                ...
                                <ul class="list-disc pl-5 space-y-1.5">
                                    <li><strong>林程揚｜Hank大叔 (加碼戰預測優勢)</strong>：完美互補的專業拋接。維琪老師極致發揮了「聽眾代理人」機制，防堵知識過於生硬，在聽眾包容度與內容增幅上拿下滿分，是知識型雙人的最高典範。</li>
...
// Insert the declaration right before const singleAwardHtml
const targetRegex1 = /const singleAwardHtml = `[\s\S]*?<div id="award-\\\${key}"/;
...
// Insert the rendering right before the table div
const targetRegex2 = /<span class="font-bold text-slate-700">📌 評選規則：<\/span>\\\${descriptionText}[\s\S]*?<\/div>\s*<\/div>\s*<div class="overflow-x-auto">/;
if (targetRegex2.test(js)) {
    js = js.replace(/(<\/div>\s*<\/div>\s*)(<div class="overflow-x-auto">)/, '$1\\${specialCommentaryHtml}\n                            $2');
