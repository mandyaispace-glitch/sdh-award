...
// 1. Replace postRequest to add stricter timeouts (though it was already there, ensure it's not the bottleneck)
code = code.replace(/function postRequest\(url, headers, body, timeoutMs = \d+\) \{[\s\S]*?req\.end\(\);\s*\n\s*\}/, 
`function postRequest(url, headers, body, timeoutMs = 300000) { // 5 minutes timeout
...
// 2. Replace downloadFile to add absolute timer
code = code.replace(/function downloadFile\(url, destPath\) \{[\s\S]*?\n\s*\}/, 
...
// 3. Add Promise.race timeout wrapper for queryVoiceAnalysisRaw
...
// 4. Update the catch block to retry instead of skipping
...
`console.error(\` ❌ 處理該單集出錯 (非配額錯誤):\`, err.message);
                    // Instead of skipping immediately, we should retry up to 3 times
